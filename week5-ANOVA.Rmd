...
```{r}
chinstrap_ttest <- t.test(x=chinstraps_f, y= chinstraps_m)
chinstrap_ttest

cohen.d(chinstraps_f,chinstraps_m)
```

2. Are male chinstrap penguins significantly larger in body mass than female chinstrap penguins? T

```{r}
# is male mass "greater" than chinstraps female
...
penguin_f_summary <- penguin_f %>% 
  ...
  summarize( mean_flipper_length = mean(flipper_length_mm),
             ...
             sample_size = n(),
             sd_flipper_length = sd(flipper_length_mm)/sqrt(n()),
...
#Pr(>F) >0.05 means we have good prob that variances are equal
```
...
```{r}
# one-way ANOVA (type I)
penguin_aov <- aov(flipper_length_mm ~ species, data=penguin_f)
summary(penguin_aov)

# find the pair that has significantly different mean
TukeyHSD(penguin_aov)
# there is a significant difference in mean among each pair of penguins
```
